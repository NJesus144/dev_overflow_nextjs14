...
const GlobalResult = () => {
  ...
  useEffect(() => {
    const fetchResult = async () => {
      ...
      try {
        ...
        console.log("resultados", JSON.parse(res))
  ...
  return (
    <div className="absolute top-full z-10 mt-3 w-full bg-light-800 py-5 shadow-sm dark:bg-dark-400 rounded-xl">
      <p className="text-dark400_light900 paragraph-ssemibold px-5">
        <GlobalFilters />
      </p>
              ...
              result.map((item: any, index: number) => (
                ...
                >
                  ...
                  <div className="flex flex-col">
                    ...
                    <p className="text-light400_light500 small-medium mt-1 font-bold">{item.type}</p>
                  </div>
                </Link>
              ))
            ) : (
              <div className="flex-center flex-col px-5">
                <p className="text-dark20_light800 body-regular px-5 py-2.5">Oops, no results found</p>
              </div>
            )}
          </div>
        )}
